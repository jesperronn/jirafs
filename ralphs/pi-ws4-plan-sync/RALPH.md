...
commands:
  ...
  - name: stream_ledger
    run: cat docs/ralph-stream-ws4-plan-sync.md
  - name: recent_commits
  ...
  - name: lint
    run: bin/lint
---
# jirafs WS4 Ralph Loop

Implement one WS4 plan/sync task, then stop.
...
WS4 task is ready, report `blocked` and exit non-zero so
`ralph run --stop-on-error` stops the loop.

Rules:

- Stay inside `internal/plan/**` and `internal/sync/**`.
