# B004d: Bash-test `bin/integrate_stream_commit` clean-worktree guard and default `main` target behavior.

## Dependencies

B004a

## Owned paths

`bin/integrate_stream_commit`, `tests/**`

## Acceptance

Bash-test `bin/integrate_stream_commit` clean-worktree guard and default `main` target behavior.

## Rules

- No placeholder code, no TODO comments, no partial implementations.
- Add tests in the same task.
- Final gates: `bin/test` and `bin/lint` must both pass.
- Commit with conventional commit wording after gates pass.
- Move this file to `tasks/done/` as part of the same commit.
